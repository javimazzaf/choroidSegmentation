...
for iter=1:length(dirlist)
    try
        ...
        if isempty(inclframelist)
          disp(logit(savedir,'ChoroidMakeFigures - Cancelled. There is no included frames.'))
          continue
        end
...
%         fvt = Output{2};
        ...
        % Plot traces from included frames
        inclFh = figure('Visible','off');
        axes('Visible','off');
        image(im,'CDataMapping','scaled')
        colormap(gray), axis off, hold all
...
%         saveas(LS_Fh,fullfile(savedir,'FrequencyCorrelationTotal.png'));
        print(fullfile(savedir,'FrequencyCorrelationTotal.png'),'-dpng','-r300')
        close(LS_Fh)
