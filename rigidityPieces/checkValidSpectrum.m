function [isValid, faProb, f, P] = checkValidSpectrum(t,signal,HR,po)
...
nFreqStepTol = getParameter('SPECTRUM_STEPS_TOLERANCE');

loF = fmin - floor(nFreqStepTol / 2) * fStep;
hiF = fmax + floor(nFreqStepTol / 2) * fStep;
...
faProb = getFalseAlarmProbability(P,1,nFreq,mxPeak);
...
function faProb = getFalseAlarmProbability(vals,corrWindow,nPks, peak)

% Computes the probability of false alarm detection
